...
class ModelMenuVMegamenu extends Model {		
	public function getMenu($module_id = 0) {
		...
		$query = $this->db->query("SELECT * FROM ".DB_PREFIX."vmega_menu WHERE parent_id='0' AND status='0' AND module_id='" . $module_id . "' ORDER BY rang");
	...
	public function getSubmenu($id, $module_id = 0) {
		...
		$query = $this->db->query("SELECT * FROM ".DB_PREFIX."vmega_menu WHERE parent_id='".$id."' AND status='0' AND module_id='" . $module_id . "' ORDER BY rang");
		foreach ($query->rows as $row) {
								...
								if(isset($array['categories'][$i]['id'])) {
									...
									if(isset($info_category['category_id'])) {
										...
										if(is_array($info_category)) {
											...
											$output .= '<li><a href="'.$link.'" onclick="window.location = \''.$link.'\';" class="main-menu ' . $class_link . '">'.$info_category['name'].'</a>';
