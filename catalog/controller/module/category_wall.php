...
class ControllerModuleCategoryWall extends Controller {
	public function index($setting) {
		...
		if(isset($setting['namecat'][$this->config->get('config_language_id')])) {
			$data['namecat_text'] = html_entity_decode($setting['namecat'][$this->config->get('config_language_id')], ENT_QUOTES, 'UTF-8');
			if($data['namecat_text'] == '') $data['namecat_text'] = 'More...';
		} else {
			$data['namecat_text'] = 'More...';
		}
		
		if(isset($setting['linkcat'][$this->config->get('config_language_id')])) {
			$data['linkcat_text'] = html_entity_decode($setting['linkcat'][$this->config->get('config_language_id')], ENT_QUOTES, 'UTF-8');
			if($data['linkcat_text'] == '') $data['linkcat_text'] = 'More...';
		} else {
			$data['linkcat_text'] = 'More...';
		}
