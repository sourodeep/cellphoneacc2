...
class ControllerModuleVMegaMenu extends Controller {
	...
	public function index() {  
		...
		$this->language->load('module/vmegamenu');

		//Set the title from the language file $_['heading_title'] string
		$this->document->setTitle('Pro Mega Menu Vertical'); 
		...
		// Instalacja vmegamenu w bazie danych
		$this->model_menu_vmegamenu->install();
		...
		foreach($this->model_menu_vmegamenu->getModules() as $module) {
			$data['vmegamenu_modules'][] = array(
				'id' => $module['id'],
				'name' => $module['name']
			);
		...
		if(isset($_GET['module_id'])) {
			if($this->model_menu_vmegamenu->getModules($_GET['module_id'])) {
				$data['active_module_info'] = $this->model_menu_vmegamenu->getModules($_GET['module_id']);
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if(isset($_POST['add-module'])){
				if($this->model_menu_vmegamenu->addModule($this->request->post)) {
					$this->session->data['success'] = $this->language->get('text_success');
				} else {
					$this->session->data['error_warning'] = $this->language->get('text_warning');
				}
				
				$this->response->redirect($this->url->link('module/vmegamenu', 'token=' . $this->session->data['token'], 'SSL'));
		    }
		}
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if(isset($_POST['delete-module'])){
				if($this->model_menu_vmegamenu->deleteModule($this->request->post)) {
					$vmegamenu = array();
					if($this->config->get('vmegamenu_module')) {
						foreach($data['vmegamenu_modules'] as $module) {
							if($module['id'] != $this->request->post['vmegamenu_modules']) {
								$module_id = $module['id'];
								$vmegamenu_module = $this->config->get('vmegamenu_module');
								if(isset($vmegamenu_module[$module_id])) $vmegamenu['vmegamenu_module'][$module_id] = $vmegamenu_module[$module_id];
							}
						}
						$this->model_setting_setting->editSetting('vmegamenu', $vmegamenu);
					}
					$this->session->data['success'] = $this->language->get('text_success');
				} else {
					$this->session->data['error_warning'] = $this->language->get('text_warning');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if(isset($_POST['active-module'])){
				if($this->request->post['vmegamenu_modules'] == 0) $this->response->redirect($this->url->link('module/vmegamenu', 'token=' . $this->session->data['token'], 'SSL'));
				$this->response->redirect($this->url->link('module/vmegamenu&module_id=' . $this->request->post['vmegamenu_modules'], 'token=' . $this->session->data['token'], 'SSL'));
		    }
		}
		...
		if(isset($_GET['delete'])) {
			if($this->validate()){
				if($this->model_menu_vmegamenu->deleteMenu(intval($_GET['delete']))) {
					$this->session->data['success'] = 'This menu has been properly removed from the database.';
				} else {
					$this->session->data['error_warning'] = $this->model_menu_vmegamenu->displayError();
				}
			} else {
				$this->session->data['error_warning'] = $this->language->get('error_permission');
			}
			
			$this->response->redirect($this->url->link('module/vmegamenu' . $url, 'token=' . $this->session->data['token'], 'SSL'));
		}
			...
			if(isset($_POST['button-create'])) {
				if($this->validate()) {
					...
					if($error == true) {
						...
						$this->model_menu_vmegamenu->addMenu($this->request->post, $data['active_module_id']);
						$this->session->data['success'] = $this->language->get('text_success');
					}
				} else {
					$this->session->data['error_warning'] = $this->language->get('error_permission');
			...
			if(isset($_POST['button-edit'])) {
				if($this->validate()) {
					...
					if($error == true) {
						...
						$this->model_menu_vmegamenu->saveMenu($this->request->post, $data['active_module_id']);
						$this->session->data['success'] = $this->language->get('text_success');
					}
				} else {
					$this->session->data['error_warning'] = $this->language->get('error_permission');
		...
		$data['nestable_list'] = $this->model_menu_vmegamenu->generate_nestable_list($data['language_id'], $data['active_module_id']);
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if(isset($_POST['button-save'])){
				...
				$vmegamenu['vmegamenu_module'][$module_id] = array(
					'module_id'  => $module_id,
		...
		if (isset($_GET['jsonstring'])) {
			if($this->validate()){
				...
				foreach ($readbleArray as $key => $value) {
					if (is_array($value)) {
						$this->model_menu_vmegamenu->save_rang($value['parentID'], $value['id'], $key, $data['active_module_id']);
					}	
				}
		...
		// Pobranie ustawień
		$ustawienia = $this->config->get('vmegamenu_module');
		...
		if(isset($_GET['edit'])) {
			...
			$dane = $this->model_menu_vmegamenu->getMenu(intval($_GET['edit']));
			if($dane) {
				$data['name'] = $dane['name'];
				...
				$data['list_categories'] = $this->model_menu_vmegamenu->getCategories($dane['content']['categories']['categories']);
			} else {
		...
		$data['action'] = $this->url->link('module/vmegamenu' . $url, 'token=' . $this->session->data['token'], 'SSL');
		...
		$data['breadcrumbs'][] = array(
			'text' => 'Pro Mega Menu Vertical',
			'href' => $this->url->link('module/vmegamenu', 'token=' . $this->session->data['token'], 'SSL')
		);
		...
		$this->response->setOutput($this->load->view('module/vmegamenu.tpl', $data));
	}
	
	private function validate() {
		if (!$this->user->hasPermission('modify', 'module/vmegamenu')) {
