...
class ModelMenuVMegamenu extends Model {
	private $errors = array();
	
	public function generate_nestable_list($lang_id, $module_id) {
		$query = $this->db->query("SELECT * FROM ".DB_PREFIX."vmega_menu WHERE parent_id='0' AND module_id='" . $module_id . "' ORDER BY rang");
		$action = $this->url->link('module/vmegamenu&module_id=' . $module_id, 'token=' . $this->session->data['token'], 'SSL');
	...
	public function menu_showNested($parentID, $lang_id, $module_id) {	
		$query = $this->db->query("SELECT * FROM ".DB_PREFIX."vmega_menu WHERE parent_id='".$parentID."' AND module_id='" . $module_id . "' ORDER BY rang");
		$action = $this->url->link('module/vmegamenu&module_id=' . $module_id, 'token=' . $this->session->data['token'], 'SSL');
		$output = false;		
	...
	public function save_rang($parent_id, $id, $rang, $module_id) {
		$this->db->query("UPDATE " . DB_PREFIX . "vmega_menu SET parent_id = '" . $parent_id . "', rang = '" . $rang . "' WHERE id = '" . $id . "' AND module_id = '" . $module_id . "'");
	}
	...
	public function addModule($data) {
		$query = $this->db->query("SELECT * FROM ".DB_PREFIX."vmega_menu_modules WHERE name='" . $data['add-module-name'] . "'");
		if(count($query->rows) < 1) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "vmega_menu_modules SET name = '" . $data['add-module-name'] . "'");
			return true;
		}
		return false;
	}
	...
	public function deleteModule($data) {
		$query = $this->db->query("SELECT * FROM ".DB_PREFIX."vmega_menu_modules WHERE id='" . $data['vmegamenu_modules'] . "'");
		if(count($query->rows) > 0) {
			$this->db->query("DELETE FROM " . DB_PREFIX . "vmega_menu_modules WHERE id = '" . $data['vmegamenu_modules'] . "'");
			$this->db->query("DELETE FROM " . DB_PREFIX . "vmega_menu WHERE module_id = '" . $data['vmegamenu_modules'] . "'");
			return true;
		}
		return false;
	}
	...
	public function getModules($module_id = 'false') {
		if($module_id != 'false') {
			$query = $this->db->query("SELECT * FROM ".DB_PREFIX."vmega_menu_modules WHERE id = '" . $module_id . "'");
		} else {
			$query = $this->db->query("SELECT * FROM ".DB_PREFIX."vmega_menu_modules");
		}
		if(count($query->rows) > 0) { 
			$data = array();
			foreach ($query->rows as $result) {
	...
	public function install() {
		if($this->is_table_exist(DB_PREFIX . "vmega_menu")) {
			$query = $this->db->query("
				CREATE TABLE IF NOT EXISTS `".DB_PREFIX."vmega_menu` (
			...
			$query = $this->db->query("
				CREATE TABLE IF NOT EXISTS `".DB_PREFIX."vmega_menu_modules` (
					`id` int(11) unsigned NOT NULL AUTO_INCREMENT,
					`name` text,
					PRIMARY KEY (`id`)
				) ENGINE=MyISAM DEFAULT CHARSET=utf8 AUTO_INCREMENT=1
